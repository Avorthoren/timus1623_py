...
def _draw_room(canvas: tkinter.Canvas, outer_room: RectTemplate, room: RectTemplate) -> RectTemplate:
	...
	canvas.create_oval(*room, outline="grey")

	return room


def _draw_door(canvas: tkinter.Canvas, outer_room: RectTemplate, door: RectTemplate):
	door = DrawTemplate.get_rect_from_abstract(door, outer_room, ensure_min=True)
	canvas.create_oval(*door, outline="black", fill="grey")
...
def draw_template(
	...
	# Draw level-0 doors.
	for door in template.outer_doors:
		_draw_door(canvas, outer_room, door)
